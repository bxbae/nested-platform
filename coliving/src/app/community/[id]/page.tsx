...
import { useLocalizedText } from "@/lib/i18n/useLocalizedText";
...
const lifestyleLabels: Record<string, Record<string, string>> = {
  ...
  smoking: {
    NON_SMOKING_ONLY: "비흡연 환경",
    OUTDOOR_OK: "실외 흡연 가능",
    SMOKING_OK: "흡연 무관",
  },
  pets: {
    NO_PETS: "반려동물 없음",
    CONDITIONAL: "반려동물 조건부",
    PETS_OK: "반려동물 환영",
  },
  visitors: {
    PRIOR_AGREEMENT: "방문객 사전 협의",
    OCCASIONAL_OK: "가끔 방문 가능",
    FREQUENT_OK: "방문객 자유",
  },
  sleep: {
    EARLY_BIRD: "아침형",
  ...
  drinking: {
    NON_DRINKER: "음주 거의 안 함",
    SOCIAL_DRINKER: "가끔 음주",
    FREQUENT: "음주 잦음",
  },
};
...
export default function PostPage() {
  ...
  const {
    text: localizedTitle,
    translating: titleTranslating,
    error: titleTranslationError,
  } = useLocalizedText(post?.title ?? "", isAuthenticated);

  const {
    text: localizedBody,
    translating: bodyTranslating,
    error: bodyTranslationError,
  } = useLocalizedText(post?.body ?? "", isAuthenticated);

  const postTranslating = titleTranslating || bodyTranslating;

  const postTranslationError = titleTranslationError ?? bodyTranslationError;
  ...
  async function sharePost() {
    ...
    if (navigator.share)
      await navigator.share({ title: post?.title ?? "Nested 커뮤니티", url });
    else {
  ...
  return (
    <div
      className="wrap"
      style={{ paddingTop: 40, paddingBottom: 60, maxWidth: 820 }}
    >
      <Link
        href="/community"
      ...
      >
        ...
        >
          <div style={{ display: "flex", gap: 7, alignItems: "center" }}>
            <span
              className="chip"
              style={{ color: catColor[post.category], fontSize: 11.5 }}
            >
              {post.category}
            </span>
              ...
              >
                {isAuthor && (
                  <>
                    ...
                    <button className="chip" onClick={() => void removePost()}>
                      삭제
                    </button>
                    <button
                      className="chip"
                ...
                <button className="chip" onClick={() => void sharePost()}>
                  링크 공유
                </button>
                {!isAuthor && (
                  <button
                    className="chip"
                    onClick={() => {
                      setReportTarget({ type: "COMMUNITY_POST", id: post.id });
                      setMenuOpen(false);
                    }}
                  >
                    신고하기
        ...
        {editing ? (
          ...
          <>
            <h1 style={{ fontSize: 26, marginTop: 16 }}>{localizedTitle}</h1>
              ...
              {Object.entries(post.lifestyleSnapshot).flatMap(([key, raw]) => {
                ...
                return values.map((v, i) => (
                  <span
                    ...
                    style={{ fontSize: 12 }}
                  >
                    {lifestyleLabels[key]?.[String(v)] ?? String(v)}
                  </span>
                ));
              })}
            </div>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        {post.comments.length === 0 && (
          <p style={{ color: "var(--text-2)" }}>아직 댓글이 없어요.</p>
        )}
        {post.comments.map((comment) => (
          <CommentCard
            ...
            translationEnabled={isAuthenticated}
            onProfile={setProfileUserId}
            onReply={(id, name) => setReplyTo({ id, name })}
            onEdit={setEditingComment}
            onDelete={removeComment}
            onReport={(id) =>
              setReportTarget({ type: "COMMUNITY_COMMENT", id })
            }
          />
        ))}
      </div>
      ...
      <div className="card" style={{ padding: 16, marginTop: 18 }}>
        {isAuthenticated ? (
          <>
            ...
            >
              {posting ? "등록 중…" : replyTo ? "답글 남기기" : "댓글 남기기"}
            </button>
          </>
        ) : (
          <Link href="/?auth=1" className="btn btn-primary">
            로그인하고 댓글 작성
          </Link>
        )}
      </div>
      ...
      {shownReplies.length > 0 && (
        ...
        >
          ...
          {hiddenCount > 0 && (
            <button
              onClick={() =>
                setVisibleReplies((count) =>
                  Math.min(count + 5, replies.length),
                )
              }
              style={{
                marginTop: 10,
...
function CommentContent({
  ...
  const {
    text: localizedCommentBody,
    translating: commentTranslating,
    error: commentTranslationError,
  } = useLocalizedText(comment.body, translationEnabled);
  return (
    <div>
      <div
